BEGIN;
    ...
    (alternatenameid, geonameid, isoLanguage, alternateName, isPreferredName, isShortName, isColloquial, isHistoric, "from", "to")
    FROM ':geonames_path/dump/alternateNamesV2.txt'
    (FREEZE TRUE, NUll '');
    ...
    COPY :geonames_schema.timeZones
    (country, timeZoneId, GMT_offset, DST_offset, RAW_offset)
    FROM ':geonames_path/dump/timeZones.txt'
    (FREEZE TRUE, NUll '');
    ...
    (code, name, geonameId)
    FROM ':geonames_path/sql/continentCodes.txt'
    (DELIMITER(','), FREEZE TRUE, NUll '');
    ...
    FROM ':geonames_path/zip/allCountries.txt'
    (FREEZE TRUE, NUll '');
